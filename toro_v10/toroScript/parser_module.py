            ...
            if type(tokens[i]) is tuple:
                if(tokens[i][0]=='operation' and tokens[i][1]==op):
                    ...
                    if(i==0):
                        ...
                        if(tokens[i-1][0]!="expr" and tokens[i-1][0]!="number" and tokens[i-1][0]!="input"):
                            doit=True
                    ...
                    if(doit==True):
                        ...
                        di={"*":[{op:[{"number":["0"]},{"number":["1"]}]},nextt]}
                        ################################ EN LA PRACTICA, SOLO PARA +-
                        newTokens.append(di)
                        i+=2
                    else:
...
def unbound(tokens):
    ...
    while i<len(tokens):
        if(type(tokens[i]) is tuple):
            if(tokens[i][0]=="string" or tokens[i][0]=="number" or tokens[i][0]=="expr"or tokens[i][0]=="input"):
